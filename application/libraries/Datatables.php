...
class Datatables
{
    ...
    private function produce_output($output, $charset)
    {
        ...
        foreach ($rResult->result_array() as $row_key => $row_val) {
            if (isset($row_val['encrypted'])) {
                $row_val['encrypted'] = id_crypt($row_val['encrypted']);
            }
            if (isset($row_val['date_formated'])) {
                $dt = new DateTime('@' . $row_val['date_formated']);
                if (isset($row_val['timezone'])) {
                    $dt->setTimeZone(new DateTimeZone($row_val['timezone']));
                }
                $row_val['date_formated'] = $dt->format('d/m/Y h:i A');
            }
